...
namespace DBreeze.Transactions
{
    public partial class Transaction : IDisposable
    {
        ...
        object _sync_vectorTransactionHandler;

        private object VectorTransactionSync
        {
            get
            {
                object sync = Volatile.Read(ref _sync_vectorTransactionHandler);
                if (sync != null)
                    return sync;
                object candidate = new object();
                return Interlocked.CompareExchange(ref _sync_vectorTransactionHandler, candidate, null) ?? candidate;
            }
        }
        ...
        private SmallWorld<float[], float>.Composer InitVectorTranF<TVector>(string tableName, VectorTableParameters<float[]> vectorTableParameters = null)
        {
            ...
            lock (VectorTransactionSync)
            {
                ...
                {
                    throw new Exception($"DBreeze. VectorLayer: Init of the table {tableName} has failed.", ex);
                }
            }
        ...
        /// <returns>Stored external identifiers and their vectors.</returns>
        /// <exception cref="NotSupportedException"></exception>
