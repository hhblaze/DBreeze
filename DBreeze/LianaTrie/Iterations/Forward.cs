...
namespace DBreeze.LianaTrie.Iterations
{
    public class Forward
    {
            ...
            byte[] startKey, bool includeStart, byte[] stopKey, bool includeStop, bool useCache)
        {
            ...
            {
                Iterator = rootNode.KidsInNode.GetKidsForward().GetEnumerator(),
                Depth = 0,
                StartRelation = 0,
                StopRelation = 0,
            });
            ...
            try
            {
                while (stack.Count > 0)
                {
                    ...
                    stack.Push(new ForwardRangeFrame
                    {
                        Iterator = node.KidsInNode.GetKidsForward().GetEnumerator(),
                        Depth = frame.Depth + 1,
        ...
        {
            return IterateForwardRangeCore(initKey, inclStartKey, stopKey, inclStopKey, useCache);
        }
