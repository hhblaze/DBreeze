...
namespace DBreeze.Storage
{
    ...
    internal class RISR : IStorage
    {
       ...
       int _seqBufCapacity = 1048576;
        ...
        private void RollbackReadExactly(byte[] buffer, int offset, int count)
        {
            while (count > 0)
            {
                int chunk = count > RemoteIoChunkSize ? RemoteIoChunkSize : count;
        ...
        private void RollbackHelperReadExactly(byte[] buffer, int offset, int count)
        {
            while (count > 0)
            {
                int chunk = count > RemoteIoChunkSize ? RemoteIoChunkSize : count;
...
//        #endregion
        ...
        public byte[] Table_WriteToTheEnd(byte[] data)
        {
            ...
            lock (lock_fs)
            {
                ...
                position = checked(RIC.DataFileLength + _seqBuf.EOF);
                encodedPosition = EncodePointer(position);
        ...
        public void Table_WriteByOffset(long offset, byte[] data)
        {
            ...
            lock (lock_fs)
            {
                ...
                {
                    if (writeEnd > checked(RIC.DataFileLength + _seqBuf.EOF))
                        throw new ArgumentOutOfRangeException("offset");
                ...
                if (offset < RIC.DataFileLength && writeEnd > RIC.DataFileLength)
                {
        ...
        private void AppendRollbackRecord(long dataOffset, int length)
        {
            ...
            RIC.DataFilePosition = dataOffset;
            DataReadExactly(record, headerLength, length);
        ...
        public byte[] Table_Read(bool useCache, long offset, int count)
        {
            lock (lock_fs)
            {
                ...
                long dataLength = RIC.DataFileLength;
                if (!useCache)
                {
                    int resultLength = GetReadLength(offset, count, checked(dataLength + _seqBuf.EOF));
                    if (resultLength == 0)
                        return new byte[0];
                    ...
                    if (offset < dataLength)
                    {
                        ...
                        DataReadExactly(result, 0, diskPart);
                ...
                RIC.DataFilePosition = offset;
                DataReadExactly(committed, 0, committed.Length);
                ...
                foreach (KeyValuePair<long, RollbackRecord> rollback in _rollbackCache)
                {
                    ...
                    RIC.RollbackFilePosition = rollback.Value.o;
            ...
            lock (lock_fs)
            {
                if (!useCache)
                {
                    ...
                    if (offset < RIC.DataFileLength)
                    {
                        ...
                        if (offset + res.Length <= RIC.DataFileLength)
                        {
                            ...
                            DataReadExactly(res, 0, res.Length);
                ...
                else
                {
                    ...
                    foreach (var bk in bufKeys)
                    {
                        ...
                        RollbackReadExactly(btWork, 0, btWork.Length);
