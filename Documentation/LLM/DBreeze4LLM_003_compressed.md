...
```csharp
tran.InsertPart<int, byte[]>("files", 42, new byte[] { 0x01, 0x02, 0x03 },
    startIndex: 16, out byte[] ptr, out bool updated);
tran.Commit();
```

### 3. Data Blocks (Blobs > 2GB)

`InsertDataBlock` returns 16-byte handle. `InsertDataBlockWithFixedAddress<T>` gives stable reference that never moves after updates.
...
**B. `Technical_SetTable_OverwriteIsNotAllowed`** — writes updates to end of file sequentially (trades temporary file bloat for speed):
...
**Why optimal:** Index stays small (8-byte keys + 16-byte pointers). `RandomKeySorter` eliminates random seeks. Updating payloads via existing `ptr16` keeps pointer stable—no tree changes needed.
</details>
...
| `GetTablePathFromTableName(string)` | Physical path or `"MEMORY"`. |
...
Set `false` when using `row.Value` outside iterator or when every row needs its value.
...
engine.Scheme.GetTablePathFromTableName("mem_temp"); // "MEMORY"
```
...
using (var t = engine.GetTransaction())
{
    var user = new User {
        ...
        Name = "Alice", Created = DateTime.UtcNow
    };
...
Each nested table requires 64-byte root in parent value. Index 0 = bytes 0–63, index 1 = bytes 64–127, etc.
...
Integrated Word Aligned Bitmap Index (WABI). Maps words to bitmap of document IDs. Usable for full-text search and multi-parameter tag search.
...
Re-inserting same external ID performs smart update (removes obsolete words, adds new).
...
```csharp
var vectors = tran.VectorsGetByExternalId<float[]>("KnowledgeBaseVectors", new List<long> { 1L }, ignoreDeleted: true);
foreach (var v in vectors)
    Console.WriteLine($"ID: {v.Item1}, Dims: {v.Item2.Length}");
```