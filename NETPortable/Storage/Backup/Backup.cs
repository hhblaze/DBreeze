    ...
    public class Backup:IDisposable
    {
        ...
        public void Dispose()
        {
            lock (lock_ibp_fs)
            {
                if (fs != null)
                {
                    IFileStream stream = fs;
                    fs = null;
                    wasWritten = false;
                    try { FSR.NET_Flush(stream); }
        ...
        private void InitBackupFolder()
        {
            try
            {
                var di = configuration.FSFactory.CreateDirectoryInfo(this._backupFolderName);
        ...
        static ulong EncodeInt64BigEndian(long value)
        {
